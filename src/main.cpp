...
uint8_t SCREEN[240][320]; // 61440 bytes
...
bool is_start_locked() {
    FIL fd;
    char pathname[256];
    sprintf(pathname, "%s\\mod-player.lock", HOME_DIR);
...
void load_config() {
    char pathname[256];
    sprintf(pathname, "%s\\mod-player.cfg", HOME_DIR);
    FIL fd;
